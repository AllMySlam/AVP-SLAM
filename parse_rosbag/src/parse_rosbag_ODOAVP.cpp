...
int main(int argc, char* argv[])
{
    ...
    for (rosbag::MessageInstance const m : rosbag::View(bag)) {
        ...
        if (s != NULL) {
            ...
            if (name == "VERTEX_SE2") {
                ...
                Odomfile << "VERTEX_SE2 " << vertexCount << " " << currentX << " " << currentY << " " << currentYaw
                         << std::endl;
            }
        }
        ...
        if (odom != nullptr && topic == "/currentPose" && GT_first == true) {
            ...
            tf::Quaternion quat(odom->pose.pose.orientation.x, odom->pose.pose.orientation.y,
                                odom->pose.pose.orientation.z, odom->pose.pose.orientation.w);
            tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);

            // save the AVP-SLAM current pose as a vertex in our pose graph
            myfile << "VERTEX_SE2 " << vertexCount << " " << odom->pose.pose.position.x << " "
                   << odom->pose.pose.position.y << " " << yaw << std::endl;
            ...
            if (vertexCount > 0) {
                ...
                myfile << "EDGE_SE2 " << vertexCount - 1 << " " << vertexCount << " " << transform_0t(0, 3) << " "
                       << transform_0t(1, 3) << " " << theta << " " << odomCovXX << " 0 0 " << odomCovYY << " 0 "
                       << odomCovTT << std::endl;
            }
        ...
        // TODO: to understand the "/odom" topic
            ...
            tf::Matrix3x3(quat).getRPY(roll, pitch, yaw);
            // only save vertexes because it will just be used for graphing and comparison purposes
            GTfile << "VERTEX_SE2 " << vertexCount << " " << odom->pose.pose.position.x << " "
                   << odom->pose.pose.position.y << " " << yaw << std::endl;
            GT_first = true;
        }
    }
    ...
    for (size_t j = ignoredVertexes; j < slamX.size(); j++) {
        ...
        // if a match was found then add the edge assuming the pose transformation is 0,0 with theta calculated as the
        // difference between j and i
        if (closestMatch != -1) {
            size_t i     = closestMatch;
            double covXX = maxCovXX * abs(slamX[i] - slamX[j]) / tooFar;
            double covYY = maxCovYY * abs(slamY[i] - slamY[j]) / tooFar;
            myfile << "EDGE_SE2 " << i << " " << j << " " << 0 << " " << 0 << " " << slamTheta[j] - slamTheta[i] << " "
                   << covXX << " 0 0 " << covYY << " 0 " << maxCovTT << std::endl;
