...
void cameraCloudHandler(const sensor_msgs::PointCloud2ConstPtr& cameraCloudMsg)
{
    ...
    std::cout << "now robot is in x " << transWorldCurrent(0, 3) << " y " << transWorldCurrent(1, 3) << "  z  "
              << transWorldCurrent(2, 3) << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    ros::Subscriber subSaveMap =
        nh.subscribe<geometry_msgs::PoseStamped>("/move_base_simple/goal", 10, saveMap_callback);
