...
int main(int argc, char* argv[])
{
    ...
    ros::Subscriber subcamera0Cloud =
        nh.subscribe<sensor_msgs::PointCloud2>("/camera0/depth/color/points", 100, camera0CloudHandler);
    ros::Subscriber subcamera1Cloud =
        nh.subscribe<sensor_msgs::PointCloud2>("/camera1/depth/color/points", 100, camera1CloudHandler);
    ros::Subscriber subcamera2Cloud =
        nh.subscribe<sensor_msgs::PointCloud2>("/camera2/depth/color/points", 100, camera2CloudHandler);
    ros::Subscriber subcamera3Cloud =
        nh.subscribe<sensor_msgs::PointCloud2>("/camera3/depth/color/points", 100, camera3CloudHandler);
    ros::Subscriber subcamera4Cloud =
        nh.subscribe<sensor_msgs::PointCloud2>("/camera4/depth/color/points", 100, camera4CloudHandler);
    ros::Subscriber subcamera5Cloud =
        nh.subscribe<sensor_msgs::PointCloud2>("/camera5/depth/color/points", 100, camera5CloudHandler);
    ...
    while (ros::ok()) {
        ros::spinOnce();
        //  synchronism point cloud of multi cameras according to timestamp
        if (!camera0CloudBuf.empty() && !camera1CloudBuf.empty() && !camera2CloudBuf.empty() &&
            !camera3CloudBuf.empty() && !camera4CloudBuf.empty() && !camera5CloudBuf.empty()) {
            ...
            if ((timeCamera0Points != timeCamera1Points) || (timeCamera0Points != timeCamera2Points) ||
                (timeCamera0Points != timeCamera3Points) || (timeCamera0Points != timeCamera4Points) ||
